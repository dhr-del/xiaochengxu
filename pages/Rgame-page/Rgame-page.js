...
          })
  ...
  over:function(){//保存玩家的选定区
    ...
    wx.request({
      url:"http://127.0.0.1:8000/save_play_info/", 
      method: 'POST',
      data: {
        PeoTou:this.data.x2,//选定区域
        beiCount:this.data.beiCount,//总倍数
        monCount2:this.data.monCount2,//筹码数
      },
      header: {
        'content-type': 'application/json' // 默认值
      },
      ...
      success: (res) => {
        if(res.statusCode == 200) {
          console.log('用户信息已成功保存到后端');
          console.log(res)//请求成功后的res
          var Peo1grade=res.data.玩家得分;
          var Peo2grade=res.data.AI得分;
          var monCount1=res.data.玩家筹码;
          var monCount2=res.data.AI筹码;
