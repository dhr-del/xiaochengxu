...
Page({
  ...
  over(){
    console.log(this.data.x1,'77777');
    console.log(this.data.x2,'88888');
    console.log(this.data.monCount1,'66666');
    console.log(this.data.monCount2,'55555');
    console.log(this.data.beiCount,'44444');
    wx.request({
      url:"http://127.0.0.1:8000/save_two_play/", //返回结算要的数据
      method: 'POST',
      data: {
        wanjia1:this.data.x1,//选定区
        wanjia2:this.data.x2,
        Peo1Mon:this.data.monCount1,//筹码
        Peo2Mon:this.data.monCount2,
        zongBei:this.data.beiCount,//总倍率
      },
      header: {
  ...
  start1(e) {
    ...
    var ThatBei=this.data.ThatBei+this.data.ThatNBei;
    ...
    while (numbers1.length < this.data.ThatCount) {
      ...
      if(ThatLun!=3){
      if(ai2==true&&ai1==false)
      this.getai1();
      if(ai2==false&&ai1==true)
      this.getai2();
      if(ai1==true&&ai2==true)
      this.over();
      }
    },
    getai1:function()
    {
      ...
      console.log(this.data.randomNumbers1)
      wx.request({
        url:"http://127.0.0.1:8000/save_bendi_info"+Lun+"/", //要返回三轮后托管的选定区
        method: 'POST',
        data: {
          RobotTou:this.data.randomNumbers1,//某轮轮投掷区
          RobotSelect:this.data.x1//前一轮的选定区
        },
        header: {
          'content-type': 'application/json' // 默认值
        },
        success: (res) => {
          console.log(this.data.randomNumbers1,'11111111某轮轮投掷区')
          console.log(this.data.x1,'11111111前一轮的选定区')
          if(res.statusCode == 200) {
            console.log('用户信息已成功保存到后端');
            ...
            this.setData({
              ...
              console.log(this.data.ThatCount,'6767676767');
          } else {
            console.error('保存到后端失败:', res);
          }
        },
    ...
    getai2:function()
    {
      var Lun=3-this.data.ThatLun;
      wx.request({
        url:"http://127.0.0.1:8000/save_bendi_info"+Lun+"/", 
        method: 'POST',
        data: {
          RobotTou:this.data.randomNumbers2,
          RobotSelect:this.data.x2
        },
        header: {
          'content-type': 'application/json' // 默认值
        },
        success: (res) => {
          if(res.statusCode == 200) {
            console.log('用户信息已成功保存到后端');
            console.log(res)//请求成功后的res
            var x2=Object.values(res.data);
            var length=Object.values(res.data);
            this.setData({
              x2:x2,
              ThisCount:length
            })
  ...
    money:function(){
    ...
        })
      },
      inputChangeMoney:function(e){//双方输入筹码数
